    ...
    <div>
        <div class="form-group" style="position: relative;">
            ...
            <div v-if="searchResults.length > 0 && licenseNumberQuery" class="search-results">
                <ul>
                    <li v-for="result in searchResults" @click="addCustomer(result)">{{ result.license_number }} - {{result.customer.name}}</li>
                </ul>
            </div>
...
export default {
    ...
    methods: {
        async searchForLicenseNumber() {
            try {
                await axios.get(window.origin + '/search-license-number', {
                    params: {
                        license_number: this.licenseNumberQuery
                    }
                }).then(response => {
